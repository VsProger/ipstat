...
#
# IP вычисляется из hostname (TS-A01, TS-B17, ...) по правилу:
#   октет = индекс_буквы*20 + номер + 40   (A=1; +40 чтобы статика была >= .61,
#   т.к. DHCP кампуса раздаёт до .60 включительно)
#   A → .61-.77 | B → .81-.97 | C → .101-.105 | D → .121-.125 | E → .141-.145
# Шаг 20 на букву вмещает до 19 машин в группе без коллизий.
#
# Запуск (от root):
#   sudo bash set-static-ip.sh             # IP из hostname
#   sudo bash set-static-ip.sh 192.168.61.70   # явный IP (если hostname нестандартный)
#
...
PREFIX="192.168.61"        # первые три октета (для вычисления IP из hostname)
GATEWAY="192.168.61.1"     # шлюз /24 (ПОДТВЕРДИТЬ у IT: ip route | grep default)
MASK="24"                  # prefix маски: сеть 192.168.61.0/24 = 255.255.255.0
...
if [[ $# -ge 1 ]]; then
  ...
  if [[ "$host_up" =~ ^TS-([A-Z])([0-9]+)$ ]]; then
    ...
    octet=$(( idx*20 + num + 40 ))   # +40: статика >= .61 (DHCP занимает .1-.60)
